...
// Setting the output schema
  ...
  if (isset($_GET['src'])) {
    ...
    if (isset($_GET['metadataPrefix'])) {
      if (isset($outputSchemas[$_GET['metadataPrefix']])) {
        $outputSchema = $_GET['metadataPrefix'];
        $xsluri = $outputSchemas[$outputSchema]['xslt'];
      }
      else {
        returnHttpError(404);
...
// The available serialisations of the output resource (output format)
    foreach ($outputFormats as $k => $v) {
      $uri = str_replace($_SERVER['QUERY_STRING'], '', $_SERVER['REQUEST_URI']) . 'metadataPrefix=' . rawurlencode($outputSchema) . '&src=' . rawurlencode($xmluri) . '&outputFormat=' . rawurlencode($k);
      $rel = 'alternate';
      if ($k == $outputFormat) {
        $rel = 'self';
      }
...
// Sending HTTP headers
    ...
    header("Content-type: " . $outputFormat . ";charset=UTF8");
  ...
  <body>
    ...
    <section id="input-box">
      <form id="api" action="." method="get">
        <h1>
          <label for="metadataPrefix">Output Schema : </label>
...
<?php foreach ($outputSchemas as $k => $v) { echo '            <option value="' . $k . '">' . $v['label'] . '</option>' . "\n"; } ?>
        ...
        <p><input type="url" required="required" autofocus="autofocus" id="src" name="src" title="Copy &amp; paste the URL of DataCite records" placeholder="Copy &amp; paste the URL of DataCite records" value="<?php echo $exampleSrcURL?>"/></p>
        <p style="float:right;">
...
?>
    ...
    <section>
      <h1>Usage notes</h1>
      <p>Copy &amp; paste the URL of a file or of an OAI-PMH request returning DataCite records.</p>
